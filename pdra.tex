...
%begin  現在までの研究状況＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
	...
	これら3つの問題を踏まえ, 今こそ理論的にも観測的にも新たな視点が必要となっている.
	
	一方インフレーションにも関係する明るいニュースとして, 2015年に LIGO/Virgo グループが初めて重力波の直接観測に成功した~[3].
	続く LISA などの重力波観測衛星計画も視野に入り, 今や\emph{重力波は現実的な観測対象}となっている.
	観測された重力波はブラックホールや中性子星の連星合体から生じたものであるが,
	太陽質量の30倍を超える重いブラックホールが見つかっていることをきっかけに, 通常とは異なる生成機構を持つ\emph{原始ブラックホール (Primordial Black Hole: PBH)} も再注目されている.
	ブラックホールは主に重い恒星の重力崩壊から形成されるが, インフレーションで大きな曲率ゆらぎが作られた場合, 宇宙初期の放射ゆらぎが直接潰れることでも形成される. これを PBH と呼ぶ.
	PBH はいまだに発見されていないが, もし観測されれば\emph{インフレーション模型への大きな情報}となる. 
	...
	1)--3)の問題を踏まえ, 理論的にはこれまでのような簡素な模型だけでなく,
	インフレーションに関わる場 (インフラトン) が複数相互作用し, さらにインフラトンが曲がった多様体を成す
	複雑な模型も視野に入れ研究する必要が出てきた. 実際複数場が関わるのは超弦理論の文脈からも自然である.
	一方観測量としての PBH について, その形成には大きなゆらぎ ($\sim1$) が必要になるのに対し, CMB や LSS のような大スケールのゆらぎは非常に小さい ($\sim10^{-5}$) ことがわかっており,
	簡素な模型で両者を実現するには不自然なパラメータの微細調整が必要だった.
	また PBH 形成に必要な大きなゆらぎを摂動論で扱う危険性も指摘されている ([5]等).
	重力波に関してはその黎明期であり, 重力波からどのようなインフレーションの情報を得られるかはまだ研究途上である.
	
	そこで1)--3)の問題を解決する方策として, 本研究では \ul{i) 複数場においてもゆらぎを非摂動的かつ自動的に計算できるストカスティック形式}に着目し理論的に整備するとともに,
	\ul{ii) 複数場による自然な PBH 形成模型}を探り, さらに \ul{iii) 重力波観測量をインフレーション特徴量と関連づける}方策を採用する.
	
	
	\vspace{3pt}
	\subject{3. 研究目的・方法および特色と独創的な点}
	
	\subsubject{i) ストカスティック形式:}
	1つ目の研究目的は\ul{ストカスティック形式 (非摂動形式) でゆらぎを計算する手法を提唱}することである.
	通常の計算方法では一様背景場のまわりでゆらぎを摂動展開して計算するが,
	ストカスティック形式ではゆらぎをブラウン運動として背景場そのものに取り入れ計算するため,
	ゆらぎを非摂動的に扱える可能性がある~[6]. 
	...
	このアルゴリズムは複数場においても自動的に曲率ゆらぎを計算でき,
	さらに摂動展開を必要としないことから PBH に必要な大きなゆらぎも制御できる,
	これまでにない申請者の独創的研究の1つである.
	...
	PBH に関して, 多段階インフレーションにおける PBH 形成や (4-(1)-6) 2次重力波の計算 (4-(1)-4) を示したのち, 
	これら多段階インフレーション模型が\emph{超弦理論の予言とも相性がいい}ことを示した (4-(1)-1).
	現在は2次重力波に関して, 初期宇宙の量子色力学 (QCD) 相転移中に生成された2次重力波が
	\emph{QCD のプラズマの性質を探査}できる可能性について研究中である.
	...
	[8] V.~Vennin and A.~A.~Starobinsky,
...
%begin  研究の特色と独創的な点＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
	...
	現在宇宙のインフレーション機構について観測的に明らかになることは一段落し,
	この先には理論的にも観測的にも新たな視点が必要となっている.
	その中で申請者はストカスティック形式と$\delta N$~形式を組み合わせ曲率ゆらぎを非摂動的に計算できるようにし,
	複雑な模型の直接解析や, PBH 等大きなゆらぎが絡む物理も問題なく取り扱うことができるようになった.
	この形式の提唱者として \emph{実際に様々な模型に適用}すること, \emph{大きなゆらぎを作り PBH 形成を議論}すること, そして
	\emph{非ガウス性等の多様な観測量に拡張}することが本研究の独創的な点である.
	
	\vspace{3pt}
	\subject{② 当該研究の位置づけ・意義}
	ストカスティック形式については申請者の研究の後にフランスの Vennin 博士が理論的に整備し, その後イギリスの Wands 教授らにより PBH への応用や ([14]等),
	オランダの Prokopec 教授らにより SR 近似を超えた適用などが議論され ([15]等), さらに再び申請者が数値的整備を進めることで,
	世界的な研究分野として芽吹きつつある. これをさらに推し進め大きな流れにすることは重要課題である.
	超弦理論からの示唆や PBH, さらに重力波も絡み, 本研究は理論とこれからの観測をつなぐ架け橋としても位置づけられる.
	
	
	\vspace{3pt}
	\subject{③ 予想されるインパクトおよび将来の見通し}
	本研究は世界的にも注目されており, 完成した際にはストカスティック形式を適用した様々なインフレーション模型の解析や,
	未知のインフレーション機構の提唱, ストカスティック形式を通した加速時空における量子ゆらぎの理論的研究など, 様々な研究が派生すると期待される.
	より正確な PBH 量の見積もり方法としてもストカスティック形式が定着する.
	さらに重力波観測から得られる初期宇宙の情報を網羅的に整備することで,
	初期宇宙の理論的研究だけでなく, 日本および世界における重力波観測計画をより一層促進することができるであろう.
	...
	\begin{footnoteSBL}
	\noindent
  	[14] C.~Pattison, V.~Vennin, H.~Assadullahi and D.~Wands,
	...
	[15] T.~Prokopec and G.~Rigopoulos,
...
%begin  研究目的＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
	...
	我々はストカスティック-$\delta N$形式を応用し, \ul{新たなインフレーション機構を発見する}ことや,
	これまで計算できなかった\ul{観測量を定量的に示す}ことを研究目的とする.
	まずは前項で述べた共変なストカスティック形式の定式化と数値計算コードの公開を着任までに完了する.
	着任後は受入研究者である Matarrese 教授とともに, \emph{ストカスティック形式におけるゆらぎの非ガウス性}の計算の定式化を行う.
	ストカスティック形式は非摂動的手法であるので, ゆらぎの最低次の情報である「大きさ」だけでなく,
	正負の非対称性や大小スケールの相関などの非ガウス性の情報も自動的に含んでいる.
	そこで「$\delta N$~形式における大小スケール相関の計算 (4-(1)-5)」をストカスティック形式に拡張することで, 新たな側面からゆらぎの非ガウス性に迫る.
	
	また数値計算コードを用い, 特に以下の2つの模型を解析する.
	1つ目はインフレーションがポテンシャル相互作用だけでなく, \emph{多様体の幾何構造に強く影響}される模型である.
	前受入研究者の Renaux-Petel 博士によって多様体が負曲率を持つ場合, 運動が不安定になり相転移を起こす可能性があることが指摘され~[9],
	以降幾何学力を使った模型が注目されている ([10]等).
	2-②でも述べたとおり相転移点付近でのゆらぎの計算にはストカスティック形式が必須であり,
	また転移点で作られる大きなゆらぎは PBH を形成する可能性があり興味深い.
	もう1つは\emph{スローロール (Slow-Roll: SR) 条件が破れる}模型である.
	SRとはポテンシャルが十分平らでインフラトンがゆっくり動く状態のことを指す.
	我々は単一場SR模型では非摂動効果が小さいことを示したが (4-(1)-9),
	SR条件を破れば単一場でもゆらぎを急激に増減させることが可能であり, 非摂動効果が大きくなってストカスティック形式が必須となる可能性が指摘されている ([11]等).
	また超弦理論の文脈ではポテンシャルは常にSR条件を破ることが示唆され~[2],
	単一場に限らず複数場においてもSR状態を超えてゆらぎの解析を行うことが求められている.
	我々の定式化はSR近似を用いない完全な形式で議論されているため,
	SR条件が破れる模型にも適用可能である.
	...
	他にも受入先であるパドヴァ大学の Peloso 教授はインフレーション中でのパリティを破る粒子生成について詳しく, 協力を仰げると期待する.
	...
	\begin{footnoteSBL}
	\noindent
  	[9] S.~Renaux-Petel and K.~Turzyński,
	...
	[11] J.~M.~Ezquiaga and J.~García-Bellido,
	...
	[12] N.~Bartolo, V.~De Luca, G.~Franciolini, A.~Lewis, M.~Peloso and A.~Riotto,
...
%begin  外国で研究する事の意義 (figureやtable使用可)＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
	...
	受入にあたっての連絡交換によれば, 現在も我々とは独立にストカスティック形式の非ガウス性への応用の研究立ち上げを考えていたようである.
	同研究室の Bartolo 教授はゆらぎの非ガウス性や大小スケール相関に詳しく ([16]等), 特に CMB や銀河ハローなどの実際の観測量に結びつける研究を多く行ってきた.
	両教授ともインフラトンとゆらぎを作る場が異なるカーバトン模型や, 時空が非等方なインフレーションなど,
	多様なインフレーション模型の現象論に詳しく ([17]等), これらに対し観測から示唆を与える研究も行ってきた.
	また PBH にも精通しており, PBH 量と初期曲率ゆらぎのパワースペクトルを結びつける研究をまとめるなどしている~[18].
	最近まで両教授は Planck 計画の理論グループとして CMB の解析を行い~[1], 特にゆらぎの非ガウス性の理論と観測の関係をまとめた ([19]等).
	現在は LISA 計画の理論グループに携わり~[20], 非ガウス性の研究を重力波観測にまで拡張している.
	
	以上のように申請者と受入研究者の研究内容の関連性は深い. 例えばストカスティック-$\delta N$の定式化においては Matarrese 教授の協力が重要であるし,
	具体的なインフレーション模型への適用や PBH 形成模型の探索, さらにはそれらの模型の観測的示唆について研究するには, 両教授のインフレーション現象論に対する
	豊富な経験が手助けとなるだろう. LISA 衛星における重力波観測に関しては Bartolo 教授の指導が不可欠である.
	またパリティを破った重力波の生成などに関しては, 同大学の Peloso 教授の協力も仰げると期待される.
	...
	[17] N.~Bartolo, S.~Matarrese and A.~Riotto,
	%``On nonGaussianity in the curvaton scenario,''
	Phys. Rev. D \textbf{69}, 043503 (2004)
	%doi:10.1103/PhysRevD.69.043503
	%[arXiv:hep-ph/0309033 [hep-ph]].
	[18] A.~Kalaja, N.~Bellomo, N.~Bartolo, D.~Bertacca, S.~Matarrese, I.~Musco, A.~Raccanelli and L.~Verde,
	...
	[19] N.~Bartolo, E.~Komatsu, S.~Matarrese and A.~Riotto,
	...
	[21] S.~Kuroyanagi, K.~Nakayama and J.~Yokoyama,
	...
	[22] V.~De Luca, G.~Franciolini and A.~Riotto,
...
%begin  研究遂行能力　＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
	...
	特筆すべきは国を超えた研究活動経験であり, 申請者は学生のころから\emph{外国人とともに共同研究}を行い ((1)-5, 6), 
	2017年度にはフランスの\emph{パリ天体物理学研究所 (IAP) にポスドク研究員として雇用}され研究を行った ((1)-2).
	現在も引き続きフランスの共同研究者と日々オンラインで研究を進めており, \emph{フランスでの国際研究会の開催協力} ((3)-1) も行っている.
	こうした国際連携活動が評価され, 今年発足したフランス日本の2国間連携企画 ``NECO" では\emph{サイエンスメンバー}に選定された.
	また2019年にはブラジル日本の2国間研究会 ``FAPESP-JSPS Workshop on dark energy, dark matter, and galaxies" にて
	\emph{若手代表発表者}として選出されサンパウロ大学にて発表を行った ((3)-2).
...
%begin  学術雑誌等に発表した論文または著書＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
	...
	\begin{enumerate} \itemsep-2mm
		...
		\item \underline{Y.~Tada} and V.~Vennin,
...
%begin  国際会議における発表＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
	...
	\begin{enumerate} \itemsep-1mm
		\item[] (口頭・招待)
		\item T.~Fujita, L.~Pinol, S.~Renaux-Petel, \underline{Y.~Tada}, J.~Tokuda, and V.~Vennin,
		``Stochastic formalism and curvature perturbation",
		3-day workshop: INFLATION AND GEOMETRY, Institute d'Astrophysique de Paris,
		2019年6月
		\item[] (口頭・査読有り)
		\item K.~Inomata, M.~Kawasaki, A.~Kusenko, K.~Mukaida, \underline{Y.~Tada}, T.~T.~Yanagida, and S.~Yokoyama,
